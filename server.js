    ...
    function(req, res) {  
        ...
        db.query(sql, [username], async function(err, results) {
            ...
            if(results.length == 0) {
                ...
                //insert customer data into the database
                sql = 'INSERT INTO employee (username, password, firstName, lastName) VALUES (?, ?, ?, ?)';
                db.query(sql, [username, password_hash, firstName, lastName], (err, result) => {
                    if (err) throw err;
...
app.post('/api/loginEM',
    async function(req, res){
        ...
        if(bcrypt.compareSync(password, password_hash)){
            ...
            //get token
            const token = jwt.sign({ employeeid: employeeid, username: username }, SECRET_KEY, { expiresIn: '1h' });                
